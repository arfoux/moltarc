...
- Decision: seal packs per-table batches to `TARGET_BYTES` (2MB); flush gates on target, hard tail rule at `MAX_BYTES` (4MB); `MIN_BYTES` (1MB) documents the small-chunk floor. `src/seal.ts:17-19`, packing/probe loop `src/seal.ts:485-549`.
- Context: mixed corpus (6000 rows, seed 7) seals 2.52MB input (decimal; 2.40MiB as displayed in `docs/bench.md`) into 3 warm chunks, 246723B (`bench/measured.json` perf; `docs/bench.md` seal 6.9MB/s in 350ms; warm bytes jitter ±~30B run to run, ratio steady at 10.2x). ~0.8MB compressed per chunk keeps find to ~1 chunk fetch per lookup (`findFetched: 11` over 6 probed ids).
- Alternatives rejected: 64KB chunks (manifest with thousands of entries, shard/sparse index bloat, ship delta fans out); 64MB chunks (find pays full read+crc+decode per lookup; a corrupt chunk quarantines 64MB instead of ~1MB, cf. `src/verify.ts:93-106` quarantine-one design).
...
- Decision: `compressFrame` tries zstd, then zstd+dict, falls back to deflate (`src/chunk.ts:120-131`); decode accepts zstd/deflate/none (`src/chunk.ts:147-155`). No LZ4 codec exists.
- Context: measured ratios come from zstd alone — 34.5x repetitive text, 10.2x mixed (`bench/measured.json` mixed; `docs/bench.md`), 1.05x on raw jpeg proving incompressible passthrough. One codec keeps `decodeHeader` + `DECOMPRESS_MAX_BYTES` cap (`src/chunk.ts:135`) a single audit surface.
...
- If reversed: LZ4 default inflates every warm chunk toward raw size; dual-codec doubles decode-path bug surface and breaks the N-2 compat error contract (`src/chunk.ts:154`).
...
- Alternatives rejected: always-train (a dict file + `DICT_FLAG` lookup per chunk for ~0% on blobs/jpegs); higher gate (8x — kills the measured 1.8% on real repetitive text); bigger dict (diminishing returns past ranked-line working set, more relay bytes via copy-if-missing `src/ship.ts:282-290`).
...
- Decision: legacy entries use `BLOOM_BITS = 2048` (`src/manifest.ts:8`), 3 hashes (`src/manifest.ts:101-114`); `bloomBitsForRows` scales newer chunks to ≥ rows×10 bits (`src/find.ts:158-164`); reader mods by actual stored length (`src/find.ts:176-191`).
- Context: perf find fetches 11 while pruning 8 across 6 probed ids (~1-2 fetches per lookup; `bench/measured.json` `findFetched: 11, findPruned: 8`) — min/max range + bloom chain (`src/find.ts:198-238`) is what keeps the warm p50 at ~13.60ms.
...
- Decision: `RESERVE_BYTES = 50MB` (`src/gc.ts:19`); `checkReserve` throws before any write on seal/merge/sweep-apply (`src/gc.ts:31-39`, `src/seal.ts:387`, `src/cold.ts:202,375`); sweep-delete-only never checks (`src/gc.ts:41-45`).
- Context: a torn chunk/watermark/manifest copy is the failure the dual-copy design exists to survive (`src/manifest.ts:548-566`); reserve guarantees the tmp+fsync+rename never starts without room for the largest expected write (one 4MB chunk + manifest copies + tar window).
...
- If reversed: seal under pressure half-writes warm chunks and manifest copies, forcing quarantine (`src/verify.ts:93-106`) and relay repair on what should have been a clean refusal.

## 6. Text-first ship lanes, blobs opt-in

- Decision: `laneOf` maps blob/photo/image/thumb tables to lane 1, everything else lane 0 (`src/ship.ts:36-43`); `planShipment` skips lane-1 unless `includeBlobs`, sorts lane then seq (`src/ship.ts:50-61`). Measured: full ship 211716B in 2 chunks default lanes; blob chunk ships only with flag (`docs/bench.md`, `bench/measured.json` perf).
...
- Decision: `findTrx` searches warm only and throws when absent (`src/find.ts:324-365`); cold needs `findCold`, which narrows by warm index then warns per scan (`src/find.ts:370-376`, warn at `src/find.ts:411-413`).
- Context: warm find is ~13.60ms p50 over 6 probed ids × 20 iters (cold-median first lookup 17.17ms; `bench/measured.json`); cold scan is O(segments) tar decode over already-zstd members (`src/find.ts:380-432`) — slower by construction.
...
- Decision: `saveManifestAtomic` writes identical payload to `manifest.json` + `manifest.bak.json` via tmp+fsync+rename, then best-effort sidecars (`src/manifest.ts:548-566`); `loadManifest` picks best crc-valid seq, primary breaks ties (`src/manifest.ts:568-592`); sparse/shard sidecars carry the same seq and fall back to root on skew (`src/find.ts:244-296`, `src/manifest.ts:288-290`).
- Context: kill mid-batch loses only the unflushed tail because the watermark advances per flushed chunk (`src/seal.ts:462-471`) and each save bumps the envelope seq — a torn primary never shadows a good backup.
- Alternatives rejected: single manifest (one torn rename = total index loss; `src/verify.ts:77-89` strict load would throw with no fallback); last-write-wins without seq (a stale primary can shadow a newer backup after a crash); WAL/journal instead (second recovery protocol for a JSON file rewritten per seal).
...
- Decision: chunks content-addressed and immutable once flushed (`src/seal.ts:506-511` oversize-still-ships comment; `src/chunk.ts` header); seal never deletes input (`src/seal.ts:2`); ship never deletes source (`src/ship.ts:3`); CAS puts never rewrite identical bytes (`src/cas.ts:40-45`); corrupt segments report-never-delete-blind (`src/cold.ts:404-415`); verify quarantines exactly one chunk (`src/verify.ts:93-106`) and repairs by hash from relay (`src/verify.ts:126-148`); sweep is dry-run by default and only deletes relay-acked orphans (`src/gc.ts:1-4,52-63`); cold prune keeps manifest rewrite atomic dual-copy (`src/cold.ts:437-451`).
- Context: delta ship is 1791B vs 211716B full (0.008, `bench/measured.json`) precisely because old chunks never change — the relay index hash hit (`src/ship.ts:56`) stays valid forever.
...
- Decision: peers exchange `summaryOf` have-lists (non-quarantined only) in hello/welcome, the learner sends a `want` delta of missing shas, the server streams meta/block/end per chunk, and the learner resumes partials from `.p2p-state-<sha12>.json` journals + `.part` files (`src/p2p.ts:95-104` summary, `src/p2p.ts:129-151` journal scan, `src/p2p.ts:607-631` hello/welcome/want, `src/p2p.ts:490-529` serve, `src/p2p.ts:231-258` storeBlock, `src/p2p.ts:171-229` applyComplete).
- Context: `summaryOf` filters quarantined chunks (`src/p2p.ts:95-104`); welcome-side skip vs want runs on a local sha set with journal offsets preserved (`src/p2p.ts:812-840`); server re-hashes bytes before serving (`src/p2p.ts:504-508`) and the learner re-hashes + `verifyChunk`s after install (`src/p2p.ts:182,202-203,225-227`); caps bound abuse (`MAX_CHUNK_BYTES` 4MB, `MAX_SESSION_BYTES` 256MB, `MAX_HAVE` 50000 at `src/p2p.ts:260-265`, enforced `src/p2p.ts:616-617,676-687,922-926`).
...
- If reversed: no have/want → every sync pays full-archive bytes; no journal → mid-transfer kills lose all partial progress; no post-transfer hash+verify → corrupt/forged wire bytes enter warm and poison the manifest merge (`src/p2p.ts:182`).
...
## 13. Sensor lossy log + claim single-spend permit (import-only reuse)

- Decision: sensor stays pure in-memory math (ts-sorted one-pass bucket downsample, two-pass z-score flags with clean-baseline re-score, hot/cold/quarantine routing, bloom-over-anomalous-ids bridge) and claim stays an in-memory exact-set permit (deterministic id, O(1) use, reconcile report, explicit persist/restore); both reuse chunk/manifest/hash primitives by import and never fork them (`src/sensor.ts:1-6` rule, `src/sensor.ts:47-77` downsample, `src/sensor.ts:87-118` flags, `src/sensor.ts:128-146` routing, `src/sensor.ts:148-156` bloom, `src/claim.ts:1-4` rule, `src/claim.ts:79-87` use, `src/claim.ts:100-108` reconcile).
- Context: first `minBaseline` points never flag and std-0 flags any deviation (`src/sensor.ts:93-99`); pass 2 excludes pass-1-flagged points from baselines so one spike cannot mask the next (`src/sensor.ts:108-116`); `anomalyToCold: false` leaves fresh-flagged points hot AND unquarantined by design (`src/sensor.ts:124-127`); claim ids are `t-<sha12(value:issuedAt:nonce)>` (`src/sensor.ts:182-184` seriesHash pattern; `src/claim.ts:37-44`); unknown ids reject before try counting (`src/claim.ts:80-83`); restart without `toJSON`/`fromJSON` re-enables spent claims (`src/claim.ts:47-49,110-114`).
- Alternatives rejected: sensor as second chunk codec (duplicates header/crc/zstd paths for summaries that already round-trip via `bucketsToRows`+`packSensor`, `src/sensor.ts:158-178`); bloom-based double-use detection (probabilistic spendability is wrong — used set is exact O(1)); auto-persisting claim store (hides the durability contract; explicit snapshot keeps the restart-loses-history rule loud).
- If reversed: sensor codec fork → summary bytes drift from canonical chunks; bloom claims → false-positive spend refusals or false-negative double-spends; silent claim durability → restarts silently re-spend claims.
...
- Decision: `openArchiveReadOnly` returns a frozen handle whose reads delegate to find/verify/gc and whose thirteen mutating ops throw a `readonly: <op> refused` error naming the dir; the module never imports seal/ship/cold write paths (`src/readonly.ts:1-3` rule, `src/readonly.ts:40-62` constructor, `src/readonly.ts:13-15` refusal).
...
- Decision: seal and p2p-receive merge caller-scanned entries into the best crc-valid manifest copy (dedupe by filename, existing entries win, filename sort, atomic dual-copy save) instead of rescanning warm (`src/manifest.ts:603-617` function; seal call `src/seal.ts:566-584`; p2p calls `src/p2p.ts:187-191,227`).
- Context: the seal watermark already covers each flushed chunk, so a kill before the append only repeats manifest work on retry (`src/seal.ts:552-557`); p2p already-applied bytes merge best-effort and return `skipped` (`src/p2p.ts:183-204`); seq+crc still bump through the same atomic save as a full rebuild (`src/manifest.ts:605,615`).
...
- Decision: wire security is a 32-byte PSK (explicit opt wins, else `MOLTARC_PSK` env) framing every message as HMAC-SHA256(json)+`.`+json verified on raw bytes BEFORE parse, plus per-message auth fields and a sha256(token) allowlist; concurrency is a `seal.lock` pid file with stale-dead-holder reclaim; downgrade safety is `assertMigrated`/`requireMigrated` on every mutating path; deletion safety is relay-acked-only gc (`src/p2p.ts:344-384` psk resolve, `src/p2p.ts:424-454` frame/unframe, `src/p2p.ts:390-423` per-message auth, `src/p2p.ts:271-276` allowlist, `src/seal.ts:313-370` lock, `src/migrate.ts:103-112` guard, `src/gc.ts:42,83-91` relay-acked sweep).
- Context: PSK accepts 64-hex / 32-raw / 32-base64 and throws on malformed non-empty values (`src/p2p.ts:304-319`); unguarded nodes warn trusted-LAN-only at startup (`src/p2p.ts:566`); node opts document the LAN fallback (`src/p2p.ts:29-36`); lock uses O_CREAT|O_EXCL before any watermark read, reclaims only on ESRCH, keeps EPERM/unparsable/same-pid locked (`src/seal.ts:345-353` create, `src/seal.ts:315-337` stale check); reads stay tolerant and bypass the migrate guard per compat N-2 (`src/migrate.ts:103-106`); gc apply without `relayDir` throws and unknown-ack orphans retain (`src/gc.ts:83-91`, orphan fields `src/gc.ts:52-58`).
