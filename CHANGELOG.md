...
- 39 audit findings fixed + pinned by regression tests (suite 246/246,
  `tsc --noEmit` clean). User-visible:
- Dedupe key is now `table:device:seq`: legit rows in different tables are no longer dropped.
- Truncated bloom fails open: live chunks are no longer pruned on a short buffer.
- Alerts fail loud: a corrupt/unreadable archive throws instead of reporting zero `ok`.
- Foto lifecycle: `foto/` joins verify + opt-in ship + sweep; readTar checks
  the ustar checksum at the tar level.
- P2P: HMAC-SHA256 per block (`MOLTARC_PSK`) + node identity; without a PSK
  the documented trusted-LAN-only fallback applies.
- `seal.lock`: a second concurrent seal fails loud instead of a watermark race.
- Migration guard: 7 write paths call `assertMigrated()`; a new binary
  refuses to rewrite an old manifest in place.
- `gc --apply` requires a relay: only relay-acked chunks are deleted (rule 7);
  without a relay the error is explicit. `forget` notes bytes stay until gc + coldg apply.
- Seal validation: negative/9-digit seq, non-finite `ts` (NaN), missing sqlite
  columns, truncated sidecars — all loud malformed at the gate.
